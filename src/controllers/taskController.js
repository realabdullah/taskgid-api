...
const parseQueryArray = (queryParam) => {
  ...
  if (Array.isArray(queryParam))
    return queryParam.filter((item) => item.trim() !== "");
  return queryParam
    .split(",")
    .map((item) => item.trim())
...
export const addTask = async (req, res) => {
  try {
    ...
    await logWorkspaceActivity(workspaceId, req.user.id, "task_created", {
      taskId: task.id,
      taskTitle: task.title,
    });
...
export const updateTask = async (req, res) => {
  try {
    ...
    const newAssigneeIds = updateData.assignees
      ? await getAssignees(updateData.assignees)
      : undefined;
    const newTagIds = updateData.tags
      ? await getTagIds(updateData.tags, workspaceId)
      : undefined;
    ...
    if (task.createdById !== req.user.id && !isAdmin) {
      return errorResponse(
        res,
        403,
        "Only the task creator or workspace admins can update this task",
      );
    }

    const originalData = task.get({ plain: true });
    const originalAssigneeIds = originalData.assignees.map(
    ...
    allowedFields.forEach((field) => {
      if (updateData[field] !== undefined) {
        if (field === "title") {
          updatePayload[field] = sanitizePlainText(updateData[field]);
        } else if (field === "description") {
          updatePayload[field] = sanitizeRichText(updateData[field]);
        } else {
    ...
    if (
      ...
      originalData.status !== updatePayload.status
    ) {
      const meta = {
        taskId: task.id,
        taskTitle: task.title,
    ...
    if (
      ...
      originalData.priority !== updatePayload.priority
    ) {
      const meta = {
        taskId: task.id,
        taskTitle: task.title,
        changeDetails: {
          field: "priority",
          from: originalData.priority,
          to: updatePayload.priority,
        },
      };
    ...
    ["title", "description", "dueDate"].forEach((key) => {
      if (
        ...
        originalData[key] !== updatePayload[key]
      ) {
        generalChanges[key] = {
    ...
    if (newAssigneeIds !== undefined) {
      const removedAssigneeIds = originalAssigneeIds.filter(
      ...
      if (addedAssigneeIds.length > 0) {
        const newEntries = addedAssigneeIds.map((userId) => ({
          taskId: task.id,
          userId,
        }));
    ...
    if (newTagIds !== undefined) {
      ...
      if (removedTagIds.length > 0) {
        await TaskTag.destroy({
          where: { taskId: task.id, tagId: { [Op.in]: removedTagIds } },
        });
      ...
      if (addedTagIds.length > 0) {
        const newTagEntries = addedTagIds.map((tagId) => ({
          taskId: task.id,
          tagId,
        }));
  ...
  try {
    ...
    if (task.createdById !== req.user.id && !isAdmin) {
      return errorResponse(
        res,
        403,
        "Only the task creator or workspace admins can delete this task",
      );
    }
  ...
  try {
    ...
    if (assignee === "me" && !currentUserId) {
      return errorResponse(
        res,
        401,
        "Authentication required to filter by tasks assigned to you.",
      );
    }
    ...
    if (statusFilter && statusFilter.length > 0) {
      ...
      const filteredStatuses = statusFilter.filter((s) =>
        validStatuses.includes(s),
      );
      if (filteredStatuses.length > 0)
        whereConditions.status = { [Op.in]: filteredStatuses };
    }

    if (priorityFilter && priorityFilter.length > 0) {
      const validPriorities = ["low", "medium", "high"];
      const filteredPriorities = priorityFilter.filter((p) =>
        validPriorities.includes(p),
      );
      if (filteredPriorities.length > 0)
        whereConditions.priority = { [Op.in]: filteredPriorities };
    }
    ...
    if (assignee) {
      const assigneesInclude = includeConditions.find(
        (inc) => inc.as === "assignees",
      );
      if (assigneesInclude) {
        if (assignee === "me") {
...
export const getTaskActivities = async (req, res) => {
  try {
    ...
    return errorResponse(
      res,
      statusCode,
      error.message || "Failed to fetch task activities",
    );
  }
};
...
export const batchAssignTasks = async (req, res) => {
  try {
    ...
    if (tasks.length === 0)
      return errorResponse(res, 404, "No valid tasks found in this workspace");
    ...
    const existingTaskIds = existingAssignments.map(
      (assignment) => assignment.taskId,
    );
    const newTaskIds = foundTaskIds.filter(
      (id) => !existingTaskIds.includes(id),
    );

    if (newTaskIds.length > 0) {
      const assignmentEntries = newTaskIds.map((taskId) => ({
        taskId,
        userId: assigneeId,
      }));
...
export const advancedSearchTasks = async (req, res) => {
  try {
    ...
    return errorResponse(
      res,
      statusCode,
      error.message || "Failed to search tasks",
    );
  }
};
...
export const exportTasksCSV = async (req, res) => {
  try {
    ...
    return errorResponse(
      res,
      statusCode,
      error.message || "Failed to export tasks to CSV",
    );
  }
};
...
export const exportTasksPDF = async (req, res) => {
  try {
    ...
    return errorResponse(
      res,
      statusCode,
      error.message || "Failed to export tasks to PDF",
    );
  }
};
