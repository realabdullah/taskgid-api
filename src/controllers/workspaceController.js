...
export const getWorkspace = async (req, res) => {
  ...
  try {
    ...
    const role = await getUserRoleInWorkspace(userId, workspace.id);
...
export const updateWorkspace = async (req, res) => {
  ...
  if (req.body.title !== undefined)
    payload.title = sanitizePlainText(req.body.title);
  if (req.body.description !== undefined)
    payload.description = sanitizeRichText(req.body.description);
  if (req.body.slug !== undefined) payload.slug = req.body.slug;
  if (req.body.logo !== undefined) payload.logo = req.body.logo;
  ...
  try {
    const payload = {
      ...
      description: req.body.description
        ? sanitizeRichText(req.body.description)
        : undefined,
      logo: req.body.logo,
      slug: req.body.slug,
      userId: req.user.id,
    ...
    await logWorkspaceActivity(workspace.id, req.user.id, "workspace_created", {
      workspaceName: workspace.title,
    });
...
export const removeTeamMember = async (req, res) => {
  ...
  try {
    ...
    const requestorRole = req.isSuperAdmin ? "creator" : "admin";
...
export const promoteToAdmin = async (req, res) => {
  ...
  try {
    ...
    await logWorkspaceActivity(workspaceId, requestorId, "member_promoted", {
...
export const demoteFromAdmin = async (req, res) => {
  ...
  try {
    ...
    await logWorkspaceActivity(workspaceId, requestorId, "member_demoted", {
...
export const getComprehensiveTeamMembers = async (req, res) => {
  ...
  try {
    ...
    const assignedTasksMap = new Map(
      tasksAssignedCount.map((item) => [
        item.userId,
        parseInt(item.totalAssigned, 10),
      ]),
    );

    const completedTasksMap = new Map(
      tasksCompletedCount.map((item) => [
        item.userId,
        parseInt(item.totalCompleted, 10),
      ]),
    );
    ...
    if (minTasksAssigned !== undefined) {
      team = team.filter(
        (member) => member.taskStats.assigned >= parseInt(minTasksAssigned, 10),
      );
    }
    if (maxTasksAssigned !== undefined) {
      team = team.filter(
        (member) => member.taskStats.assigned <= parseInt(maxTasksAssigned, 10),
      );
    }
    if (minTasksCompleted !== undefined) {
      team = team.filter(
        (member) =>
          member.taskStats.completed >= parseInt(minTasksCompleted, 10),
      );
    }
    if (maxTasksCompleted !== undefined) {
      team = team.filter(
        (member) =>
          member.taskStats.completed <= parseInt(maxTasksCompleted, 10),
      );
    }

    let totalCount;
    if (
      minTasksAssigned !== undefined ||
      maxTasksAssigned !== undefined ||
      minTasksCompleted !== undefined ||
      maxTasksCompleted !== undefined
    ) {
...
export const getTeamStatistics = async (req, res) => {
  ...
  try {
    ...
    const tasksWithDueDate = allTasks.filter(
      (task) =>
        task.dueDate &&
    ...
    for (const task of tasksWithDueDate) {
      if (task.status === "done") {
        ...
        if (
          ...
          new Date(statusChangeActivity.createdAt) <= new Date(task.dueDate)
        ) {
          onTimeDeliveries++;
        }
      }
    ...
    const onTimeDeliveryRate =
      totalTasksWithDueDate > 0
        ? (onTimeDeliveries / totalTasksWithDueDate) * 100
        : 0;
    ...
    for (const task of allTasks) {
      for (const assignee of task.assignees) {
        ...
        memberTaskAssignments.set(
          assigneeId,
          (memberTaskAssignments.get(assigneeId) || 0) + 1,
        );
      }
    }
    ...
    const teamUtilizationRate =
      memberIds.length > 0 ? (activeMembers / memberIds.length) * 100 : 0;
    ...
    for (const member of teamMembers) {
      ...
      const memberCompletedTasksCount = memberTasks.filter(
        (task) => task.status === "done",
      ).length;
      const memberCompletionRate =
        memberTasksCount > 0
          ? (memberCompletedTasksCount / memberTasksCount) * 100
          : 0;

      let memberOnTimeDeliveries = 0;
      const memberTasksWithDueDate = memberTasks.filter(
        (task) =>
          task.dueDate &&
          new Date(task.dueDate) >= startDate &&
          new Date(task.dueDate) <= now,
      ...
      const memberOnTimeRate =
        memberTasksWithDueDate.length > 0
          ? (memberOnTimeDeliveries / memberTasksWithDueDate.length) * 100
          : 0;
      ...
      for (const task of memberTasks.filter((t) => t.status === "done")) {
        ...
        if (statusChangeActivity) {
          ...
          const completionTimeHours =
            (completionTime - creationTime) / (1000 * 60 * 60);
      ...
      const avgCompletionTimeHours =
        tasksWithCompletionTime > 0
          ? totalCompletionTimeHours / tasksWithCompletionTime
          : 0;
    ...
    const avgTeamCompletionTimeHours =
      memberStats.reduce(
        (sum, stats) => sum + stats.avgCompletionTimeHours,
        0,
    ...
    const avgTasksPerMember =
      taskAssignmentCounts.length > 0
        ? taskAssignmentCounts.reduce((sum, count) => sum + count, 0) /
          taskAssignmentCounts.length
        : 0;

    // Calculate standard deviation to determine task distribution evenness
    const taskCountVariance =
      taskAssignmentCounts.length > 0
        ? taskAssignmentCounts.reduce(
            (sum, count) => sum + Math.pow(count - avgTasksPerMember, 2),
            0,
          ) / taskAssignmentCounts.length
        : 0;
    ...
    const taskDistributionEvenness =
      avgTasksPerMember > 0
        ? Math.max(0, 100 - (taskDistributionStdDev / avgTasksPerMember) * 100)
        : 100;

    // Calculate average priority level of tasks
    const priorityMap = { low: 1, medium: 2, high: 3 };
    const prioritySum = allTasks.reduce(
      (sum, task) => sum + (priorityMap[task.priority] || 0),
      0,
    );
    ...
    const data = {
      ...
      overallStats: {
        ...
        avgCompletionTimeHours: parseFloat(
          avgTeamCompletionTimeHours.toFixed(2),
        ),
        taskDistributionEvenness: parseFloat(
          taskDistributionEvenness.toFixed(2),
        ),
      ...
      topPerformers: topPerformers.map((performer) => ({
        ...
        avgCompletionTimeHours: parseFloat(
          performer.avgCompletionTimeHours.toFixed(2),
        ),
      })),
      memberStats: memberStats.map((stat) => ({
        ...
        avgCompletionTimeHours: parseFloat(
          stat.avgCompletionTimeHours.toFixed(2),
        ),
      })),
    };
...
export const exportWorkspaceDataCSV = async (req, res) => {
  try {
    ...
    if (includeTasks === "true") {
      ...
      tasks.forEach((task) => {
        const assignees = task.assignees
          .map((a) => `${a.firstName} ${a.lastName} (${a.username})`)
          .join("; ");
    ...
    if (includeMembers === "true") {
      ...
      csvContent += `Username,First Name,Last Name,Email,Role,Title,Location,Date Joined,Task Count,Completed Tasks\n`;
      ...
      const taskCountMap = new Map(
        taskCounts.map((tc) => [tc.userId, parseInt(tc.totalTasks, 10)]),
      );
      const completedCountMap = new Map(
        completedTaskCounts.map((tc) => [
          tc.userId,
          parseInt(tc.completedTasks, 10),
        ]),
      );
    ...
    if (includeActivities === "true") {
      ...
      activities.forEach((activity) => {
        ...
        const details = JSON.stringify(activity.details || {}).replace(
          /"/g,
          '""',
        );
