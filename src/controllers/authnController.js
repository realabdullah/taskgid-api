...
export const authLimiter = rateLimit({
  windowMs: process.env.NODE_ENV === "production" ? 15 * 60 * 1000 : undefined,
  max: 5,
  message: {
...
if (!rpName || !rpId || !rpOrigin) {
  throw new Error(
    "Missing required environment variables for WebAuthn configuration",
  );
}
  ...
  try {
    ...
    if (savedAuthns && savedAuthns.length >= MAX_DEVICES_PER_USER) {
      return errorResponse(
        res,
        400,
        `Maximum number of devices (${MAX_DEVICES_PER_USER}) reached`,
      );
    }

    const excludeCredentials = savedAuthns
      ? savedAuthns.map((cred) => ({
          id: cred.id,
          type: "public-key",
...
export const verifyAuthnResponse = async (req, res) => {
  try {
    ...
    const now = Date.now();
    if (
      req.user.challengeTimestamp &&
      now - req.user.challengeTimestamp > CHALLENGE_TIMEOUT
    ) {
      return errorResponse(res, 400, "Challenge expired, please try again");
    }
    ...
    if (existingDevice) {
      return errorResponse(
        res,
        400,
        "This device or security key is already registered.",
      );
    }
    ...
    await Authn.create({
      ...
      device: {
        type: deviceInfo?.type
          ? deviceInfo.type
          : ["Macintosh"].includes(deviceInfo.model)
            ? "desktop"
            : "mobile",
...
export const loginWithAuthn = async (req, res) => {
  try {
    ...
    const now = Date.now();
    if (
      user.challengeTimestamp &&
      now - user.challengeTimestamp > CHALLENGE_TIMEOUT
    ) {
      return errorResponse(res, 400, "Challenge expired, please try again");
    }
...
export const removeAuthn = async (req, res) => {
  try {
    ...
    if (authn.userId !== req.user.id) {
      return errorResponse(
        res,
        403,
        "You do not have permission to remove this authenticator",
      );
    }
